...
class BrowserHub {
  ...
  async start(profileId: string) {
    ...
    await this.ensureStarted(profile)
  ...
  async open(profileId: string, url: string, waitUntil: 'load' | 'domcontentloaded' | 'networkidle' = 'domcontentloaded') {
    const profile = this.profile(profileId)
    const page = await this.page(profile)
  ...
  async manualLogin(profileId: string) {
    const profile = this.profile(profileId)
    await this.ensureStarted(profile)
    return {
      ...(await this.describeStatus(profile, true)),
      message: profile.manualLoginMessage ?? 'Start the browser if needed, open the noVNC URL, finish the login or verification in the persistent browser profile, then tell the bot you are done.',
    }
  }
  ...
  private async page(profile: BrowserProfileConfig) {
    await this.ensureStarted(profile)
  ...
  private async browser(profile: BrowserProfileConfig, keepAlive = true) {
    ...
    await this.ensureStarted(profile, keepAlive)
  ...
  private async peekPage(profile: BrowserProfileConfig) {
    try {
      const browser = await this.browser(profile, false)
      const context = this.defaultContext(browser)
  ...
  private async ensureStarted(profile: BrowserProfileConfig, keepAlive = true) {
    ...
    if (!runtime.startPromise) {
      runtime.startPromise = this.spawnProfile(profile)
        .finally(() => {
  ...
  private async spawnProfile(profile: BrowserProfileConfig) {
    const runtime = this.runtime(profile.id)
    const child = spawn(profile.launchCommand, profile.launchArgs, {
      stdio: 'ignore',
    })
...
async function main() {
  ...
  for (const profile of config.profiles) {
    ...
    server.registerTool(`${profile.id}_start`, {
      description: `Start the on-demand browser for ${profile.label} and keep its profile data.`,
      inputSchema: {},
    ...
    server.registerTool(`${profile.id}_manual_login`, {
      description: `Start the browser for ${profile.label} and return noVNC access details for manual login or verification.`,
      inputSchema: {},
    }, async () => textResult(await hub.manualLogin(profile.id)))
  }
